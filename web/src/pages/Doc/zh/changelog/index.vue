...
<p>4.修复富文本模式下切换主题，概要节点的文本样式没有跟随改变的问题；</p>
<p>5.修复同时选中概要节点和其所属节点，设置节点样式后概要节点会失去激活状态的问题；</p>
<p>6.修复性能模式下全选节点后添加图标时，画布外的节点会报错的问题；</p>
...
<p>2.支持设置关联线的样式，即实线或虚线；</p>
<p>3.支持鼠标选中备注浮层的内容；</p>
<p>4.备注里的超链接改为新窗口打开；</p>
<p>5.修复激活节点时在缩放备注输入框中输入会输入到节点文本中的问题；</p>
<p>6.修复更改基础样式后切换主题，选择覆盖，再修改基础样式时之前覆盖的样式会恢复的问题；</p>
</blockquote>
<h2>0.10.5</h2>
<blockquote>
<p>2024.8.2</p>
...
<p>本次更新主要是增加了一个性能模式，开启后只会渲染画布可视区域内的节点，超出区域的节点会从画布删除，提升大数据量（1000+节点）情况下的可用性，2000节点首次渲染时间由5s+减少到0.5s，当然由此也会带来一些问题，比如拖动画布时会有一点卡顿，因为会实时渲染节点，导出图片等会变慢，因为需要先渲染所有节点，以及其他一些暂时未发现的问题。</p>
<p>修复：</p>
<blockquote>
<p>1.删除无用代码，修复鼠标悬浮节点上报错不显示展开收起按钮的问题；</p>
</blockquote>
<h2>0.10.3</h2>
<blockquote>
<p>2024.7.19</p>
...
<p>全选、删除节点激活相邻节点、多选节点等操作增加派发before_node_active事件；</p>
<p>多选节点改为实时派发节点激活事件；</p>
<p>修复非富文本模式下同时存在图标和换行的文本时，被收起和展开时图标与文字距离会逐渐拉大的问题；</p>
<p>修复节点富文本内容显示时的xss漏洞；</p>
</blockquote>
<p>新增：</p>
<blockquote>
<p>支持解析md文件中带html格式的标题文本；</p>
<p>演示模式中禁止画布的所有内容（除了节点的超链接和备注图标）响应鼠标事件；</p>
<p>演示模式支持填空模式，即带下划线的文本默认不显示，按回车键才会依次显示；</p>
<p>演示模式支持概要内容；</p>
<p>render类的expandToNodeUid（展开到指定节点）方法支持概要节点；</p>
<p>render类的findNodeByUid（根据uid找到画布指定节点实例）方法支持概要节点；</p>
<p>新增添加节点附加的前置和后置内容的实例化选项，即允许在节点文本前面或后面添加自定义的内容；</p>
<p>新增派发节点标签的点击事件；</p>
...
<p>修复备注浮层显示时点击收起按钮收起节点后，备注浮层未消失的问题；</p>
</blockquote>
<h2>0.9.11</h2>
<p>修复：</p>
<blockquote>
...
<p>5.修复概要节点文本编辑中按回车结束时，相应的节点高亮框会错位显示的问题；</p>
<p>6.修复同时创建多个实例时，文本编辑后节点宽高丢失的问题；</p>
<p>7.修复删除当前激活的节点的所有子节点后，展开收起按钮没有消失的问题；</p>
...
<p>4.修复 mac 上 useLeftKeySelectionRightKeyDrag 选项设为 true 时无法按住 ctrl 键多选节点的问题。</p>
<p>5.修复节点富文本编辑时清除样式后文字样式丢失并且切换主题也不会恢复的问题。</p>
<p>6.修复删除含有子节点的节点时，data_change_detail 事件报错的问题。</p>
</blockquote>
<p>新增：</p>
<blockquote>
...
<p>3.修复在微前端框架子应用中使用时，new MouseEvent 代码执行报错的问题。</p>
...
<p>8.修复当画布容器距浏览器窗口左上角不为 0 时，view.fit 方法计算出来的位置有误差的问题。</p>
<p>9.修复当节点数量比较多时，导出的图片中水印没有完全覆盖整个图片的问题。</p>
<p>10.修复插入公式报错的问题。</p>
<p>11.修复节点文本编辑状态中鼠标选择文本时移出编辑框，文字选中状态会丢失的问题。</p>
<p>12.修复节点富文本编辑时，文本选中范围为 null 时没有触发 rich_text_selection_change 事件的问题。</p>
<p>13.修复 node_dragging 事件回参里没有 node 实例的问题。</p>
<p>14.修复节点数量过多，画布尺寸过大无法导出 png 的问题。</p>
</blockquote>
<p>新增：</p>
<blockquote>
...
<p>9.重构 pdf 的导出逻辑，导出的 pdf 尺寸不再是固定的 a4，而是思维导图的尺寸，同时删除分页导出的配置。</p>
...
<p>修复：</p>
<blockquote>
<p>1.修复自定义节点内容时导出图片、svg、pdf 报错的问题。</p>
...
<p>5.在鼠标滚轮行为为上下移动画布时，支持按住 Ctrl 键进行放大缩小画布。</p>
</blockquote>
...
<p>修复：</p>
<blockquote>
<p>1.修复历史记录数据中概要节点的激活状态未被删除的问题，会导致点击概要节点时触发 data_change 事件。</p>
<p>2.修复在 safari 浏览器中运行时，页面空白且控制台抛出异常的问题。</p>
<p>3.修复缩放画布时图标浮层和备注浮层和节点脱离的问题。</p>
<p>4.修复只读模式下可以全选节点的问题。</p>
<p>5.修复富文本模式下节点内容存在 时导出为图片出错的问题。</p>
<p>6.修复先给自身添加概要，再给下级添加概要会出现概要重叠的问题；修复同时给存在上下级关系的节点添加概要时概要重叠的问题。</p>
...
<p>3.导出 png 的方法新增压缩参数；优化大数据量节点导出 pdf 时体积过大的问题。</p>
<p>4.将节点实例的 isParent 方法改名为 isAncestor，同时新增 isParent 方法。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
<p>1.修复只读模式下仍旧可以搜索替换和编辑大纲的问题。</p>
<p>2.修复节点内容为 html 标签时大纲无法显示和编辑的问题。</p>
<p>3.修复同时选中多个节点添加图标时，所有节点图标都会统一为第一个节点的图标的问题。</p>
<p>4.导出操作增加 loading。</p>
</blockquote>
<h2>0.8.0-fix.1</h2>
<p>修复：修复直接粘贴的方式创建新节点时如果粘贴的内容带有&lt;&gt;等 html 标签符号时新创建的节点内容为空的问题。</p>
<h2>0.8.0</h2>
...
<p>3.修复插入概要、上移、下移、一键整理布局的快捷键操作没有触发 data_change 事件的问题。</p>
<p>4.修复存在水印时导出图片、svg、pdf 时每个节点都会显示边框的问题。</p>
...
<p>新增：</p>
<blockquote>
<p>1.优化画布 DOM 结构，将节点、连线、关联线分层渲染。</p>
<p>2.优化水印插件。</p>
<p>3.setTheme、setThemeConfig、setLayout 函数增加不触发重新渲染的参数。</p>
<p>4.新增插入父节点的命令。</p>
<p>5.新增仅删除当前节点的命令。</p>
<p>6.插入概要时自动展开子节点。</p>
...
<p>11.指定时间内只允许添加一次历史记录，避免添加没有必要的中间状态。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
<p>1.修改回到根节点的方法及文案。</p>
...
<p>8.将节点唯一标识由 id 全部改为 uid，主要影响关联线，之前版本的关联线可能无法正常显示。</p>
...
<h2>0.7.0</h2>
<p>破坏性更新：删除了主题文件中节点激活样式的部分，不再支持设置节点的激活样式，激活效果改为统一的节点外边框样式，同时支持鼠标 hover 效果。</p>
<p>修复：</p>
<blockquote>
...
<p>9.插件新增销毁前生命周期函数，解决销毁思维导图时插件的一些副作用没有清除的问题。</p>
<p>10.优化基础样式的设置，修改不影响大小的主题属性时不触发全量渲染。</p>
<p>11.右键多选节点结束时禁止触发节点右键菜单事件，避免触发右键菜单显示。</p>
<p>12.优化 Select 插件，如果多选节点没有变化，那么不触发激活激活事件。</p>
<p>13.node_active 事件抛出的激活节点列表不再直接引用内部激活列表。</p>
<p>14.优化鼠标按下节点事件逻辑，在右键拖拽画布模式下支持右键按住根节点拖拽画布。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
<p>1.不直接引用内部激活节点列表，优化性能。</p>
<p>2.支持配置是否显示滚动条。</p>
...
<p>2.优化导出图片逻辑，遍历节点转换图片的 url 时，如果已经是 data:URL 形式不重复处理。</p>
</blockquote>
<p>新增：</p>
<blockquote>
<p>1.去除导出 svg 方法的第二个参数，改为通过实例化配置。</p>
<p>2.导出图片不再使用外部库。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
...
<h2>0.6.15-fix.2</h2>
<p>修复：修复在 Firefox 浏览器中富文本节点无法显示的问题。</p>
<h2>0.6.15-fix.1</h2>
<p>新增：</p>
<blockquote>
...
<p>2.导出 svg 的图形的 paddingX 和 paddingY 改为单侧 padding。</p>
...
<p>新增：</p>
<blockquote>
<p>1.去除创建隐藏输入框，通过 navigator.clipboard 实现复制粘贴；支持跨浏览器粘贴思维导图节点数据；支持自定义处理剪贴板中的文本数据。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
...
<p>3.修复在移动端激活节点、展开收起时等操作时会拉起输入法的问题。</p>
<p>4.修复主题配置中背景图片为 none 时会发起一个异常请求的问题。</p>
</blockquote>
<p>新增：</p>
<blockquote>
<p>1.移动端手势缩放优化: 按线性关系进行缩放、双指位移可以调整画布位置。</p>
<p>2.去掉异步渲染节点的逻辑，提升创建新节点的速度。</p>
<p>3.导出图片由 html2canvas 库改为使用 dom-to-image-more 库，解决导出富文本节点文字样式丢失的问题。</p>
<p>4.非富文本输入框进入编辑状态时取消默认全选。</p>
<p>5.存在一个激活节点时，支持按下中文、数字、英文按键时自动进入文本编辑模式。</p>
</blockquote>
<p>Demo：</p>
<blockquote>
<p>1.保存视图数据时增加防抖操作，优化性能。</p>
...
<p>4.修复当思维导图距浏览器窗口左上角不为 0 时，小地图渲染不正确的问题。</p>
...
<p>新增：</p>
<blockquote>
<p>1.节点移动结束事件（node_dragend）增加回调参数，可以获取到移动到节点的 uid。</p>
...
<p>Demo：1.新增主题、主题列表新增 tab 区分。 2.节点图片上传支持输入网络图片地址。 3.节点图片上传支持输入网络图片。</p>
<h2>0.6.10</h2>
...
<p>Demo：1.支持通过图标按钮调出搜索。 2.支持通过图标按钮切换暗黑模式。 3.优化搜索:鼠标不在搜索区域内不聚焦,解决鼠标不在搜索区域内无法删除输入的文字的问题。 4.调整添加节点图标的界面 UI，新增系列节点图标。 5.新增贴纸列表。 6.修复在搜索框回车后输入框焦点丢失的问题。 7.支持点击节点内的图标显示一个图标快捷替换和删除悬浮面板。</p>
<h2>0.6.9-fix.1</h2>
<p>修复：1.修复搜索进行一次单个替换后再全部替换不正确的问题。</p>
<p>新增：1.不会再直接修改传入的 data 对象，内部会深拷贝一份。</p>
<h2>0.6.9</h2>
<p>修复：1.修复给概要节点设置样式概要节点会消失的问题。2.修复自定义节点内容时，二次创建根实例时节点内容不渲染的问题。3.修复节点处于编辑中时添加新节点时新节点的焦点丢失问题。 2.修复连续按 tab 键无法连续创建子节点的问题。</p>
<p>新增：1.导出 svg 时替换 svg 中存在的<code>&amp;nbsp;</code>字符，避免导出的 svg 报错。 2.支持搜索和替换。</p>
<p>Demo：1.切换主题时支持选择是否覆盖设置过的基础样式。</p>
<h2>0.6.8</h2>
<p>修复：1.修改插入概要的快捷键为 Ctrl+G，避免和保存快捷键冲突。 2.修复节点正在编辑时切换富文本编辑配置输入框出现异常的问题。</p>
...
<p>新增：1.支持根据节点实例或节点 uid 定位到某个节点。 2.修改节点 uid 的创建方式，导出数据添加节点的 uid。</p>
<p>移除：1.移除节点过渡效果。</p>
<p>Demo：1.添加网站首页。 2.修复大纲里创建新节点时节点样式丢失的问题。 3.修复大纲里编辑节点后按回车或 Tab 键后编辑文本丢失的问题。 4.优化大纲的节点定位，被收起的节点会自动展开。 5.侧边栏按钮支持收起。 6.优化小屏适配。</p>
<h2>0.6.6</h2>
<p>新增：1.支持导出为 Xmind 新版文件。2.导入 Xmind 新版文件支持导入节点中的图片。 3.新增竖向时间轴结构。</p>
<p>修复：1.TouchEvent 插件不再派发 click 事件，解决移动端点击超链接会打开两个窗口的问题。 2.修复拖拽移动一个节点成为另一个节点的子节点时该节点的父节点指向未更新的问题。 3.修复二级节点拖拽成三级节点时节点边框样式未更新的问题。 4.修复向右生长的结构外其他结构鼠标移入展开收起按钮位置时不会触发按钮显示的问题。</p>
<p>优化：1.优化触控板缩放画布时幅度过大的问题。2.刚创建的节点默认全选方便删除默认文本。</p>
<h2>0.6.5-fix.1</h2>
<p>修复：1.修复在缩放情况下调整图片大小不正确的问题。</p>
<h2>0.6.5</h2>
<p>修复：1.修复 xmind 文件导入报错的问题。 2.修复极少数情况下当节点文本的宽度为小数时显示发生换行的问题。</p>
...
<h2>0.6.0</h2>
<p>破坏性更新：调整了 simple-mind-map 源码的目录结构，主要影响：1.插件的引入路径需要修改。2.constant 文件路径需要修改。</p>
<p>新增：1.支持一键缩放至适应画布功能。 2.按住 Ctrl 键多选功能可通过配置按需开启。 3.支持设置为左键多选节点，右键拖动画布。 4.支持控制节点是否允许编辑。 5.新增销毁思维导图的方法。 6.新增触摸事件支持插件。</p>
<p>修复：1.修复按住 ctrl 键多选节点时不会触发节点的 click 事件的问题。 2.修复清空一个节点后再输入文字时节点样式丢失的问题。</p>
<h2>0.5.11</h2>
<p>新增：支持关联性文本编辑。</p>
<p>优化：优化主题配置更新，改变不涉及节点大小的配置不触发节点重新计算。</p>
<h2>0.5.10</h2>
<p>新增：使用 LRU 缓存算法优化节点复用逻辑。</p>
<h2>0.5.10-fix.1</h2>
<p>修复：修复导入出错的问题。</p>
<h2>0.5.10-fix.2</h2>
...
<h2>0.5.7</h2>
<p>破坏性更新：富文本模式下导出 png 改为使用 html2canvas 转换整个 svg，大幅提高导出速度，不过 html2canvas 存在一个 bug，foreignObject 元素中的 dom 节点内联的文字颜色无法识别，所以导出节点的文字颜色是固定的，不过相对于之前的导出基本不可用状态，目前至少能快速顺利的导出。</p>
<p>优化：优化富文本节点编辑体验。</p>
...
<h2>0.5.5</h2>
<p>新增：1.支持配置导出为 png、svg、pdf 时的内边距。 2.支持配置节点文本编辑框、节点备注浮层元素的 z-index。 3.支持点击画布外的区域结束节点编辑状态。</p>
<h2>0.5.5-fix.1</h2>
...
<p>修复：1.修复节点右键和画布右键的冲突问题。 2.修复组织结构图、目录组织图等节点拖拽时存在线段未隐藏的 bug。</p>
...
<p>新增：1.支持导入和导出 Markdown 格式文件。 2.支持配置插入节点时的初始文字。 3.扩展插入节点和删除节点的命令，支持指定节点。</p>
<h2>0.4.6</h2>
<p>新增：1.关联线支持调整控制点。</p>
<p>优化：1.添加历史数据时过滤和上一次相比没有改变的数据。</p>
<p>修复：1.修复节点编辑时方向键和方向键导航功能的冲突问题。 2.修复拖拽移动节点时节点 id 的丢失问题，这会导致关联线丢失。</p>
<h2>0.4.5</h2>
<p>新增：1.支持关联线。 2.按住根节点也可以拖动画布。3.按住 ctrl 键可以调整多选节点。</p>
<h2>0.4.4</h2>
<p>新增：支持响应鼠标的横向滚动。</p>
<h2>0.4.3</h2>
...
<p>修复：1.Mac 系统触控板缩放相反的问题；2.设备 window.devicePixelRatio 不为 1 时，当存在富文本节点时导出的图片中富文本节点尺寸会变大的问题。</p>
<h2>0.4.0</h2>
<p>新增：节点支持富文本编辑。</p>
<h2>0.3.4</h2>
<p>新增：节点文本增加自动换行功能。</p>
...
<p>新增：支持节点横线风格。</p>
<h2>0.2.20</h2>
<p>修复：画布距窗口左上角不为 0 时节点拖拽出现偏移的问题。</p>
<h2>0.2.19</h2>
...
<h2>0.2.8</h2>
<p>修复：xmind8 版本文件导入失败的问题。</p>
<p>新增：支持展开到指定层级。</p>
<h2>0.2.7</h2>
<p>修复：根节点添加多个节点爆栈的问题。</p>
<p>新增：支持导入.xmind 文件。</p>
<h2>0.2.6</h2>
<p>新增：导出 svg 增加 title 标签。</p>
<h2>0.2.5</h2>
<p>修复：节点展开收起的 bug。</p>
<p>新增：节点支持自定义线条样式。</p>
<h2>0.2.4</h2>
...
<p>新增：支持导出为 pdf。</p>
<h2>0.2.0</h2>
<p>新增：经典 4 主题；支持添加概要；支持自由拖拽；上移节点、下移节点、复制节点、剪切节点、粘贴节点、一键整理布快捷键；库打包；Ctrl+左键多选。</p>
<h2>0.1.18</h2>
<p>修复：节点图标不能删除的问题；工具按钮置灰仍然可以点击的问题。</p>
<h2>0.1.17</h2>
<p>新增：增加只读模式。</p>
<h2>0.1.16</h2>
<p>新增：节点备注支持 markdown 及富文本。</p>
