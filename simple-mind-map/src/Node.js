...
import { asyncRun, resizeImgSize } from './utils'
import { A, Circle, G, Image, Rect, SVG, Text } from '@svgdotjs/svg.js'
...
class Node {
  ...
  constructor(opt = {}) {
    ...
    //是否存在自定义文本
    this.hasCustomText =
      this.nodeData.data.customText && this.nodeData.data.customText.length
  }
  ...
  createTextNode() {
    ...
    if (this.hasCustomText) {
      ...
      const preCustomText = this.nodeData.data.customText
        .filter(item => item.position === 'pre')
        .sort((pre, cur) => pre.sort - cur.sort)
      const postCustomText = this.nodeData.data.customText
        .filter(item => item.position === 'post')
        .sort((pre, cur) => pre.sort - cur.sort)
      ...
      customText.forEach((item, index) => {
            ...
            this.nodeData.data.text
              ...
              .forEach((rootItem, rootIndex) => {
                ...
                if (direction === 'horizontal' || !direction) {
                  node.x(totalWidth)
                  totalWidth +=
                    this.measureText(rootItem, fontSize).width + spacing
                } else if (direction === 'vertical') {
                  node.y(totalHeight + rootY)
                  totalHeight +=
                    this.measureText(rootItem, fontSize).height + spacing
                }
                g.add(node)
              })
        ...
        let customTextFontSize = this.getStyle(
          'fontSize',
          this.isRoot,
          this.nodeData.data.isActive
        )
        let customTextLineHeight = this.getStyle(
          'lineHeight',
          this.isRoot,
          this.nodeData.data.isActive
        )
        ...
        item.content.split(/\n/gim).forEach((childItem, childIndex) => {
          ...
          if (item.x && item.y) {
            ...
            node.x(totalWidth)
            totalWidth +=
              this.measureText(childItem, customTextFontSize).width + spacing
          } else if (direction === 'vertical') {
        ...
        if (
          (index + 1 < customText.length &&
            customText[index + 1].position !== 'pre') ||
          index === customText.length - 1
        ) {
          setRootData()
        }
      })
  ...
  getSelfInhertStyle(prop) {
    return (
      this.getSelfStyle(prop) || this.getParentSelfStyle(prop) // 自身
    ) // 父级
  }
