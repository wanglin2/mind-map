...
class Render {
    ...
    registerCommands() {
        ...
        // 修改节点自定义文本样式
        this.setNodeCustomTextStyle = this.setNodeCustomTextStyle.bind(this)
        this.mindMap.command.add('SET_NODE_STYLE', this.setNodeCustomTextStyle)
        // 切换节点是否激活
    ...
    //设置节点自定义文本样式 -TODO
    setNodeCustomTextStyle(node, prop, value, isActive, index) {
        let data = {}
        if (isActive) {
            data = {
                activeStyle: {
                    ...(node.nodeData.data?.customText[index].activeStyle || {}),
                    [prop]: value
                }
            }
        } else {
        ...
        this.setNodeDataRender(node, data, 'custom', index)
    }
