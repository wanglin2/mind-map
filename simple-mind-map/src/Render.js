...
class Render {
  ...
  setLayout() {
    this.layout = new (layouts[this.mindMap.opt.layout]
      ? layouts[this.mindMap.opt.layout]
      : layouts.logicalStructure)(this)
  }
    ...
    if (symbol === 'origin')
      ...
      Object.keys(data).forEach(key => {
        if (
          ...
          node.nodeData.data.customText[index]['styleConfig'][key] = data[key]
        else node.nodeData.data.customText[index][key] = data[key]
      })
    else if (symbol === 'addCustomData')
      node.nodeData.data.customText = [
        ...node.nodeData.data.customText,
        ...data
      ]
  }
